...
public class TbsStaticPlugin : FlutterPlugin, MethodCallHandler, ActivityAware {
    ...
    companion object {
        ...
        fun registerWith(registrar: Registrar) {
            ...
            registrar.platformViewRegistry().registerViewFactory(
                "com.tbs_static/x5WebView",
                X5WebViewFactory(registrar.messenger(), registrar.activity(), registrar.view())
            )
        }
    }
    ...
    override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
        if (call.method == "getPlatformVersion") {
            ...
            var ok = QbSdk.canLoadX5(mActivity)
            if (ok) {
                result.success(ok)
            } else {
        ...
        } else if(call.method == "getCrashInfo") {
    ...
    override fun onAttachedToActivity(binding: ActivityPluginBinding) {
        ...
        mFlutterPluginBinding?.platformViewRegistry?.registerViewFactory(
            "com.tbs_static/x5WebView",
            X5WebViewFactory(mFlutterPluginBinding?.binaryMessenger!!, mActivity!!, null)
        )
    }
